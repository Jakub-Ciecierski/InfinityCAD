...
void BSpline::drawBezierSpline(const glm::mat4 &VP, const Color& color){
    ...
    for(int i = 0; i < segmentCount; i++){
        ...
        ic::Point* bezierPoint0 = curve.getPoint(0);
        ic::Point* bezierPoint1 = curve.getPoint(1);
        ic::Point* bezierPoint2 = curve.getPoint(2);
        ic::Point* bezierPoint3 = curve.getPoint(3);
...
void BSpline::drawPolygon(const glm::mat4 &VP, int SEGMENTS){
    ...
    for(unsigned int i = 0; i < points.size() - 1; i++){
        ...
        const ic::Point* point1 = points[i];
        const ic::Point* point2 = points[i+1];
...
void BSpline::buildBezierSpline(){
    ...
    for(int i = 0; i < segmentCount; i++){
        ...
        if(i == 0) {
            ic::Point *point0 = objectFactory.createPoint("tmp");
            point0->moveTo(deboorPos0);
            bezierPoints.push_back(point0);
        }
        ic::Point* point1 = objectFactory.createPoint("tmp");
        point1->moveTo(deboorPos1);
        bezierPoints.push_back(point1);
        ic::Point* point2 = objectFactory.createPoint("tmp");
        point2->moveTo(deboorPos2);
        bezierPoints.push_back(point2);
        ic::Point* point3 = objectFactory.createPoint("tmp");
...
const std::vector<RenderBody *> &BSpline::getChildren() {
    if(bezierSpline != NULL)
        return bezierSpline->getChildren();
    else
        return RenderBody::getChildren();
}
